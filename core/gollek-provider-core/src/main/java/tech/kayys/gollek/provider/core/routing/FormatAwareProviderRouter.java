...
public class FormatAwareProviderRouter {
    ...
    private StreamingProvider selectProvider(ProviderRequest request) {
        ...
        // 1. If preferred provider is specified and it supports the model, use it
        // strictly.
    ...
    private List<StreamingProvider> orderedProviders(ProviderRequest request) {
        ...
        // Detect Stable Diffusion pipeline to steer away from text-only safetensor
        // provider.
        // We resolve the model path via the registry first to handle relative
        // IDs/aliases.
        boolean isSd = false;
        try {
        ...
        // If it's a Stable Diffusion model, move 'onnx' to the front unless it's
        // already there
