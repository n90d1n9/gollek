...
public class ModelRegistryService implements ModelRegistry {
    ...
    private ModelManifest buildManifest(
            ...
            String checksum,
            Long sizeBytes) {
        Map<tech.kayys.gollek.core.model.ModelFormat, tech.kayys.gollek.spi.model.ArtifactLocation> artifacts = buildArtifacts(
                request.framework(),
                storageUri,
                checksum,
                sizeBytes);
    ...
    private Map<tech.kayys.gollek.core.model.ModelFormat, tech.kayys.gollek.spi.model.ArtifactLocation> buildArtifacts(
            String format,
            String storageUri,
            String checksum,
            Long sizeBytes) {
        ...
        tech.kayys.gollek.core.model.ModelFormat resolved = resolveFormat(format, storageUri);
        if (resolved == null) {
            return Collections.emptyMap();
        }
    ...
    private tech.kayys.gollek.core.model.ModelFormat resolveFormat(String format, String storageUri) {
        if (format != null && !format.isBlank()) {
            String normalized = format.trim().toLowerCase();
            if ("litert".equals(normalized) || "litert".equals(normalized)) {
                return tech.kayys.gollek.core.model.ModelFormat.LITERT;
            }
            return tech.kayys.gollek.core.model.ModelFormat.findByRuntime(format)
                    .or(() -> tech.kayys.gollek.core.model.ModelFormat.findByExtension(format))
                    .orElse(null);
        }
        ...
        if (storageUri != null) {
            ...
            if (dot >= 0 && dot + 1 < path.length()) {
                ...
                return tech.kayys.gollek.core.model.ModelFormat.findByExtension(ext).orElse(null);
            }
        }
