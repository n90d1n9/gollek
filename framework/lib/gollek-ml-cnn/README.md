...
3. **Training Pipeline**: Works with:
   - gollek-ml-optimize (optimizers)
   - gollek-ml-train (trainers, schedulers)
   - gollek-ml-data (data loading)

## Example: Building a Simple CNN

```java
import tech.kayys.gollek.lib.cnn.layers.*;
...
```bash
mvn test -f gollek/sdk/lib/gollek-ml-cnn/pom.xml
```
...
- `gollek-ml-autograd` - For automatic differentiation
- `gollek-ml-nn` - For neural network utilities
- `gollek-ml-vision` - For backend dispatch and vision utilities
