...
The `gollek-ml-hub` connects local storage mechanisms and serialization logic to the runtime layers.

## Responsibilities
- Maps weights from binary files (like `.safetensors`) directly into off-heap structured memory via `gollek-runtime-tensor`.
- Organizes the `state_dict` loading mechanism, translating tensor arrays onto initialized `Module`s in `gollek-ml-nn`.
