...
public final class Functions {
    ...
    public static final class Add {
        public static GradTensor apply(GradTensor a, GradTensor b) {
            ...
            GradTensor out;
            if (a.storage().type() == TensorStorage.Type.UNIFIED && b.storage().type() == TensorStorage.Type.UNIFIED && backend.priority() > 0) {
                out = new GradTensor(new UnifiedStorage(a.storage().size()), a.shape(), a.dtype(), a.device(), false);
                backend.add(a.storage().asSegment(), b.storage().asSegment(), out.storage().asSegment(), a.shape());
            } else {
                ...
                out = GradTensor.of(result, a.shape());
            }
    ...
    public static final class Sub {
        public static GradTensor apply(GradTensor a, GradTensor b) {
            ...
            if (a.storage().type() == TensorStorage.Type.UNIFIED && b.storage().type() == TensorStorage.Type.UNIFIED && backend.priority() > 0) {
                out = new GradTensor(new UnifiedStorage(a.storage().size()), a.shape(), a.dtype(), a.device(), false);
                backend.sub(a.storage().asSegment(), b.storage().asSegment(), out.storage().asSegment(), a.shape());
            } else {
                float[] ad = a.data(), bd = b.data();
                float[] result;
                if (bd.length == ad.length) {
    ...
    public static final class Mul {
        public static GradTensor apply(GradTensor a, GradTensor b) {
            ...
            if (a.storage().type() == TensorStorage.Type.UNIFIED && b.storage().type() == TensorStorage.Type.UNIFIED && backend.priority() > 0) {
                out = new GradTensor(new UnifiedStorage(a.storage().size()), a.shape(), a.dtype(), a.device(), false);
                backend.mul(a.storage().asSegment(), b.storage().asSegment(), out.storage().asSegment(), a.shape());
            } else {
                float[] ad = a.data(), bd = b.data();
                float[] result;
                if (bd.length == ad.length) {
    ...
    public static final class Div {
        public static GradTensor apply(GradTensor a, GradTensor b) {
            ...
            if (a.storage().type() == TensorStorage.Type.UNIFIED && b.storage().type() == TensorStorage.Type.UNIFIED && backend.priority() > 0) {
                out = new GradTensor(new UnifiedStorage(a.storage().size()), a.shape(), a.dtype(), a.device(), false);
                backend.div(a.storage().asSegment(), b.storage().asSegment(), out.storage().asSegment(), a.shape());
            } else {
                float[] ad = a.data(), bd = b.data();
                float[] result;
                if (bd.length == ad.length) {
    ...
    public static final class Matmul {
        ...
        public static GradTensor apply(GradTensor a, GradTensor b) {
            ...
            float[] ad = null, bd = null;
            if (a.storage().type() == TensorStorage.Type.UNIFIED && b.storage().type() == TensorStorage.Type.UNIFIED && backend.priority() > 0) {
                out = new GradTensor(new UnifiedStorage((int)GradTensor.numelFor(outShape)), outShape, a.dtype(), a.device(), false);
                backend.matmul(a.storage().asSegment(), as, b.storage().asSegment(), bs, out.storage().asSegment());
            } else {
                ad = a.data(); 
                bd = b.data();
                float[] result = backend.matmul(ad, as, bd, bs);
                out = GradTensor.of(result, outShape);
            }
            if (a.requiresGrad() || b.requiresGrad()) {
                if (ad == null) ad = a.data();
                if (bd == null) bd = b.data();
                final float[] fAd = ad, fBd = bd;
                out.requiresGrad(true);
                    ...
                    public void backward(GradTensor upstream) {
                            ...
                            for (int bIdx = 0; bIdx < fBatch; bIdx++) {
                                ...
                                for (int i = 0; i < fM; i++) {
                                    for (int k = 0; k < fK; k++) {
                                        ...
                                        int j = 0;
                                        int loopBound = SPECIES.loopBound(fN);
                                        for (; j < loopBound; j += SPECIES.length()) {
                                            FloatVector uVec = FloatVector.fromArray(SPECIES, ug, ugOff + i * fN + j);
                                            FloatVector bVec = FloatVector.fromArray(SPECIES, fBd, bOff2 + k * fN + j);
                                            sumVec = sumVec.add(uVec.mul(bVec));
                                        }
                                        float s = sumVec.reduceLanes(jdk.incubator.vector.VectorOperators.ADD);
                                        for (; j < fN; j++) {
                                            s += ug[ugOff + i * fN + j] * fBd[bOff2 + k * fN + j];
                                        }
                                        ga[gaOff + i * fK + k] += s;
                                    }
                                }
                        ...
                        if (b.requiresGrad()) {
                            ...
                            for (int bIdx = 0; bIdx < fBatch; bIdx++) {
                                ...
                                for (int i = 0; i < fM; i++) {
                                    for (int k = 0; k < fK; k++) {
                                        float a_ik = fAd[aOff2 + i * fK + k];
                                        int j = 0;
                                        int loopBound = SPECIES.loopBound(fN);
                                        for (; j < loopBound; j += SPECIES.length()) {
                                            FloatVector gVec = FloatVector.fromArray(SPECIES, gb, gbOff + k * fN + j);
                                            FloatVector uVec = FloatVector.fromArray(SPECIES, ug, ugOff + i * fN + j);
                                            gVec.add(uVec.mul(a_ik)).intoArray(gb, gbOff + k * fN + j);
                                        }
                                        for (; j < fN; j++) {
                                            gb[gbOff + k * fN + j] += a_ik * ug[ugOff + i * fN + j];
                                        }
                                    }
                                }
                            }
            ...
            for (int o = 0; o < outerStride; o++) {
                for (int i = 0; i < innerStride; i++) {
                    ...
                    for (int v = 0; v < dimSize; v++) {
                        sum += in[o * dimSize * innerStride + v * innerStride + i];
                    }
                    out[o * innerStride + i] = sum;
                }
            }
            
            GradTensor res = GradTensor.of(out, outShape);
            if (input.requiresGrad()) {
                res.requiresGrad(true);
                final int fOuterStride = outerStride;
                final int fInnerStride = innerStride;
                final int fDimSize = dimSize;
                final float[] fIn = in;
                res.setGradFn(new Function.Context("SumAxis") {
                    @Override
                    public void backward(GradTensor upstream) {
                        float[] ug = upstream.data();
                        float[] grad = new float[fIn.length];
                        for (int o = 0; o < fOuterStride; o++) {
                            for (int i = 0; i < fInnerStride; i++) {
                                float val = ug[o * fInnerStride + i];
                                for (int v = 0; v < fDimSize; v++) {
                                    grad[o * fDimSize * fInnerStride + v * fInnerStride + i] = val;
                                }
                            }
                        }
                        input.backward(GradTensor.of(grad, input.shape()));
    ...
    public static final class Mean {
        ...
        public static GradTensor apply(GradTensor input, int dim) {
            ...
            GradTensor sum = Sum.apply(input, dim);
            GradTensor res = sum.div((float) dimSize);
            return res;
        }
    }
