...
public final class TensorOps {
    ...
    /** Handles [B, M, K] @ [B, K, N] or [B, H, I, J] @ [B, H, J, D] */
    private static GradTensor batchedMatmul(GradTensor a, GradTensor b) {
        if (a.ndim() == 3) return batchedMatmul3d(a, b);
        if (a.ndim() == 4) return batchedMatmul4d(a, b);
        throw new UnsupportedOperationException("batchedMatmul: unsupported ndim: " + a.ndim());
    }

    private static GradTensor batchedMatmul3d(GradTensor a, GradTensor b) {
    ...
    private static GradTensor batchedMatmul4d(GradTensor a, GradTensor b) {
        long[] sa = a.shape(), sb = b.shape();
        int B = (int) sa[0], H = (int) sa[1], I = (int) sa[2], J = (int) sa[3];
        int D = (int) sb[3];
        float[] ad = a.data(), bd = b.data();
        float[] out = new float[B * H * I * D];
        for (int batch = 0; batch < B; batch++) {
            for (int head = 0; head < H; head++) {
                int aOff = (batch * H + head) * I * J;
                int bOff = (batch * H + head) * J * D;
                int outOff = (batch * H + head) * I * D;
                float[] aSlice = slice1d(ad, aOff, I * J);
                float[] bSlice = slice1d(bd, bOff, J * D);
                float[] c = VectorOps.matmul(aSlice, bSlice, I, J, D);
                System.arraycopy(c, 0, out, outOff, I * D);
            }
        }
        return GradTensor.of(out, B, H, I, D);
    }
