...
public final class FuseAddMulPass implements GraphOptimizer {
    ...
    public Node optimize(Node root) {
        if (!(root instanceof UnaryOpNode u))
            return root;
        if (!(u instanceof UnaryOpNode mulNode))
            return root;
        if (!(mulNode instanceof UnaryOpNode))
            return root;
        Node input = ((UnaryOpNode) root).inputs()[0];
        if (input instanceof BinaryOpNode addNode) {
            if (addNode.op() instanceof AddOp &&
                    ((UnaryOpNode) root).op() instanceof MulScalarOp mulOp) {
