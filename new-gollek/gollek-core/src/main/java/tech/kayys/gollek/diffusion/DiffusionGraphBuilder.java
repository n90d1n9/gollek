...
public final class DiffusionGraphBuilder {
    public Node build(
        ...
        for (int i = 0; i < timesteps.length; i++) {
            Node epsCond = new UNetNode(latents, cond, timesteps[i], model);
            Node epsUncond = new UNetNode(latents, uncond, timesteps[i], model);
            Node guided = new CFGNode(epsCond, epsUncond, guidanceScale);
            latents = new SchedulerNode(latents, guided, scheduler, i);
        }
        return latents;
    }
}