...
public class OpenAIProvider implements StreamingProvider {
    ...
    public Uni<ProviderHealth> health() {
        return Uni.createFrom().item(() -> {
            ...
            return ProviderHealth.healthy(
                    currentApiKey != null && !currentApiKey.isBlank() ? "OpenAI API available"
                            : "OpenAI initialized (API key missing)");
        });
    }
            ...
            return Uni.createFrom()
                    ...
                    .map(resp -> {
                        if (resp.statusCode() != 200) {
                            throw new RuntimeException("OpenAI failed: " + resp.statusCode() + " " + resp.body());
                        }
                        try {
                            com.fasterxml.jackson.databind.JsonNode root = objectMapper.readTree(resp.body());
                            OpenAIResponse response = objectMapper.treeToValue(root, OpenAIResponse.class);
                            ...
                            throw new RuntimeException("OpenAI deserialization failed", e);
                        }
                    });
        } catch (Exception e) {
    ...
    public Multi<StreamingInferenceChunk> inferStream(ProviderRequest request) {
        ...
        log.debug("OpenAI streaming URL: " + url);
        ...
        try {
            ...
            return Multi.createFrom().emitter(emitter -> {
                httpClient.sendAsync(httpRequest, HttpResponse.BodyHandlers.ofInputStream())
                        .thenAccept(resp -> {
                            if (resp.statusCode() != 200) {
                                emitter.fail(new RuntimeException("OpenAI streaming failed: " + resp.statusCode()));
                                return;
                            }
                                    ...
                                    if (line.startsWith("data: ")) {
                                        ...
                                        if (!data.isEmpty() && !"[DONE]".equals(data)) {
                                            try {
                                                ...
                                                emitter.emit(StreamingInferenceChunk.of(request.getRequestId(), index,
                                                        content));
                                            } catch (Exception e) {
    ...
    private OpenAIRequest buildOpenAiRequest(ProviderRequest request) {
        ...
        String model = request.getModel() != null ? request.getModel().trim() : "gpt-3.5-turbo";
        openAiRequest.setModel(model);

        if (request.getMessages() != null) {
            List<OpenAIMessage> messages = request.getMessages().stream()
                    .map(msg -> new OpenAIMessage(msg.getRole().toString().toLowerCase(), msg.getContent()))
        ...
        if (request.getParameters() != null) {
            if (request.getParameters().containsKey("temperature")) {
                openAiRequest.setTemperature(((Number) request.getParameters().get("temperature")).doubleValue());
            }
            ...
            if (request.getParameters().containsKey("frequency_penalty")) {
                openAiRequest
                        .setFrequencyPenalty(((Number) request.getParameters().get("frequency_penalty")).doubleValue());
            }
            if (request.getParameters().containsKey("presence_penalty")) {
                openAiRequest
                        .setPresencePenalty(((Number) request.getParameters().get("presence_penalty")).doubleValue());
            }
        }
