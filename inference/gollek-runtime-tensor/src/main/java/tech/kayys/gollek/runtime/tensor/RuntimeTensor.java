 ...
 * throughout the inference pipeline. All inference engines (LibTorch, GGML,
 * ONNX
 ...
 * When closed, native memory is either freed or returned to a
 * {@link TensorPool}
 * for reuse, enabling zero-allocation execution during graph inference.</li>
 * <li><strong>Zero-Copy Views:</strong> View operations (reshape, slice,
 * squeeze)
 * create new tensor instances without copying underlying data.</li>
 * <li><strong>Quantization Support:</strong> Native support for quantized
 * formats
 ...
 * <h2>Memory Layout</h2>
 * <p>
 * Tensors use a strided memory layout where each dimension has an associated
 * stride
 * defining the number of elements to skip to advance to the next position. The
 * actual
 ...
 * <h2>Thread Safety</h2>
 * <p>
 * Tensor implementations are not required to be thread-safe. Concurrent access
 * to
 * the same tensor instance must be externally synchronized. However, different
 * tensor
 ...
 * @since 1.0
 */
public interface RuntimeTensor extends AutoCloseable {
