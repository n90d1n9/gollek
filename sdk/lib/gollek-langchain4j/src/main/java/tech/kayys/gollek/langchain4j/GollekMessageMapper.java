            ...
            if (message instanceof UserMessage userMessage && userMessage.contents() != null) {
                for (Content content : userMessage.contents()) {
                    if (content instanceof TextContent) {
                        ...
                        // ImageContent can have base64 or URL. ContentPart handles strings by checking URL/Base64 scheme.
                        if (imageContent.image().base64Data() != null) {
                            parts.add(ContentPart.image("data:image/png;base64," + imageContent.image().base64Data()));
                        } else if (imageContent.image().url() != null) {
                            parts.add(ContentPart.image(imageContent.image().url().toString()));
                        }
                    }
                }
            } else {
