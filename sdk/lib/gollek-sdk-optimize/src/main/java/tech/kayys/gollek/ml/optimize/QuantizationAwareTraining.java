 ...
 *
 * <p>Unlike post-training quantization, QAT typically recovers 1-2% accuracy
 * that PTQ loses, at the cost of additional training time.
 *
 * <p>Strategy: insert fake-quantize operations in the forward pass that
...
public final class QuantizationAwareTraining {
     ...
     */
    public QuantizationAwareTraining(Module model, Optimizer optimizer) {
        this.model     = model;
     ...
     */
    public void enableQAT() { this.qatEnabled = true; }

    /**
     * Disables fake-quantization (returns to normal float32 training).
     */
    public void disableQAT() { this.qatEnabled = false; }

    /**
     * Forward pass with optional fake-quantization of weights.
     *
     * <p>When QAT is enabled, weights are fake-quantized to INT8 precision
    ...
    public GradTensor forward(GradTensor input) {
        if (qatEnabled) fakeQuantizeWeights();
     ...
     */
    public void step() { optimizer.step(); }
     ...
     * Gradients flow through as if the operation were identity (straight-through estimator).
     */
