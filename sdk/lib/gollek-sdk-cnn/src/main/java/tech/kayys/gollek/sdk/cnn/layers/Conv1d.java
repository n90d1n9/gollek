 ...
 *
 * <p>Applies a 1D convolution over an input signal of shape [N, C_in, L].</p>
 *
 * <h3>Example</h3>
 * <pre>{@code
 * Conv1d conv = new Conv1d(3, 16, 3, 1, 1);  // in_channels=3, out_channels=16, kernel=3, stride=1, padding=1
 * GradTensor x = GradTensor.randn(2, 3, 100);  // batch_size=2, channels=3, length=100
 * GradTensor y = conv.forward(x);  // Shape: [2, 16, 100]
 * }</pre>
 *
 * @author Gollek Team
 * @version 0.1.0
...
public class Conv1d {
    ...
    public Conv1d(int inChannels, int outChannels, int kernelSize,
        ...
        // Initialize weight: [out_channels, in_channels, kernel_size]
        double limit = Math.sqrt(6.0 / (inChannels * kernelSize + outChannels));
        this.weight = GradTensor.uniform(outChannels, inChannels, kernelSize, -limit, limit);
    ...
    /**
     * Create Conv1d with default dilation=1 and bias=true.
     */
    ...
    public GradTensor forward(GradTensor input) {
        ...
        return backend.conv1d(input, weight, bias ? biasParam : null, stride, padding);
    }
    ...
    public String toString() {
        return String.format("Conv1d(%d, %d, kernel_size=%d, stride=%d, padding=%d, dilation=%d)",
