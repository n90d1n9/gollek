...
public final class ResNet {
    ...
    private static final class ResNetModel extends NNModule {
        ...
        private Module[] buildBlocks(int inC, int outC, int count, int stride, boolean bottleneck) {
            Module[] blocks = new Module[count];
            blocks[0] = bottleneck
                ? new BottleneckBlock(inC, outC, stride)
                : new ResidualBlock(inC, outC, stride);
            for (int i = 1; i < count; i++)
                blocks[i] = bottleneck
                    ? new BottleneckBlock(outC, outC, 1)
                    : new ResidualBlock(outC, outC, 1);
            return blocks;
        }
    }
