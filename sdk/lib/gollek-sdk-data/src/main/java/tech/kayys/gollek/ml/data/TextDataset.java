 ...
 * var dataset = TextDataset.fromDirectory(Path.of("data"), tokenizer, maxLength=128);
 * }</pre>
 *
 * <h3>Example — language modeling</h3>
 * <pre>{@code
 * var dataset = TextDataset.fromFile(Path.of("corpus.txt"), tokenizer, chunkSize=512);
 * }</pre>
 */
public final class TextDataset implements Dataset {

    private final List<Sample> samples;

    private TextDataset(List<Sample> samples) { this.samples = samples; }

    @Override public int size()              { return samples.size(); }
    @Override public Sample get(int index)   { return samples.get(index); }

    /**
     * Loads a text classification dataset from a directory.
     *
     * <p>Each subdirectory is treated as a class label (sorted alphabetically).
     ...
     */
    public static TextDataset fromDirectory(
            Path dir,
            tech.kayys.gollek.ml.nlp.Tokenizer tokenizer,
            ...
            try (var files = Files.walk(classDirs.get(classIdx), 1)) {
                files.filter(p -> p.toString().endsWith(".txt")).forEach(file -> {
                    try {
                        ...
                        var enc = tokenizer.batchEncode(List.of(text), maxLength, true, true);
        ...
        return new TextDataset(samples);
    }

    /**
     * Loads a language modeling dataset from a single text file.
     *
     * <p>Tokenizes the entire file and splits into fixed-length chunks.
     ...
     */
    public static TextDataset fromFile(
            Path file,
            tech.kayys.gollek.ml.nlp.Tokenizer tokenizer,
            int chunkSize) throws IOException {

        String text = Files.readString(file);
        List<Integer> allIds = tokenizer.encode(text);
        List<Sample> samples = new ArrayList<>();

        for (int i = 0; i + chunkSize + 1 <= allIds.size(); i += chunkSize) {
            float[] input  = new float[chunkSize];
            float[] target = new float[chunkSize];
            for (int j = 0; j < chunkSize; j++) {
                input[j]  = allIds.get(i + j);
                target[j] = allIds.get(i + j + 1);
            }
            samples.add(new Sample(
