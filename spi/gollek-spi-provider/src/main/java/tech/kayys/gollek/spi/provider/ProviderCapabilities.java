...
public final class ProviderCapabilities {
    ...
    public ProviderCapabilities(
        ...
        this.supportedFormats = normalizeFormats(supportedFormats);
    ...
    /**
     * Normalizes legacy format enums coming from older provider artifacts.
     * Some packaged providers still emit `tech.kayys.gollek.spi.model.ModelFormat`,
     * while the active runtime expects `tech.kayys.gollek.core.model.ModelFormat`.
     */
    private static Set<ModelFormat> normalizeFormats(Set<?> supportedFormats) {
        if (supportedFormats == null || supportedFormats.isEmpty()) {
            return Collections.emptySet();
        }
        Set<ModelFormat> normalized = new LinkedHashSet<>();
        for (Object candidate : supportedFormats) {
            if (candidate == null) {
                continue;
            }
            if (candidate instanceof ModelFormat format) {
                normalized.add(format);
                continue;
            }
            if (candidate instanceof Enum<?> legacyEnum) {
                try {
                    normalized.add(ModelFormat.valueOf(legacyEnum.name()));
                    continue;
                } catch (IllegalArgumentException ignored) {
                    // Fall through to the string-based path below.
                }
            }
            try {
                normalized.add(ModelFormat.valueOf(String.valueOf(candidate).trim().toUpperCase()));
            } catch (IllegalArgumentException ignored) {
                // Ignore unknown legacy values instead of crashing provider discovery.
            }
        }
