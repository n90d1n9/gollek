...
void at_copy_(void* dst_tensor_ptr, void* src_tensor_ptr) {
    torch::NoGradGuard guard;
    static_cast<at::Tensor*>(dst_tensor_ptr)->copy_(*static_cast<at::Tensor*>(src_tensor_ptr));
}
...
void* at_slice(void* tensor_ptr, long dim, long start, long end, long step) {
    return new at::Tensor(static_cast<at::Tensor*>(tensor_ptr)->slice(dim, start, end, step));
}
...
void at_save_state_dict(const char** keys, void** tensor_ptrs, long count, const char* path) {
    torch::serialize::OutputArchive archive;
    for (long i = 0; i < count; i++) {
        archive.write(std::string(keys[i]), *static_cast<at::Tensor*>(tensor_ptrs[i]));
    }
    archive.save_to(path);
}

void at_load_state_dict(const char* path, const char** keys, void** tensor_ptrs, long count) {
