...
/// Flutter plugin implementation for Gollek TFLite inference engine on macOS.
...
public class GollekEngineFlutterPlugin: NSObject, FlutterPlugin {
    ...
    private func handleDestroyEngine(args: [String: Any]?, result: @escaping FlutterResult) {
        ...
        // Clean up any associated streaming sessions
        for (key, _) in streamingSessions.filter({ $0.key.hasPrefix("\(engineId)_stream") }) {
    ...
    private func handleStartStreaming(args: [String: Any]?, result: @escaping FlutterResult) {
        ...
        let maxTokens = args?["maxTokens"] as? Int ?? 0
