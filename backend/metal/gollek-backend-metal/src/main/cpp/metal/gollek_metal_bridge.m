...
int gollek_metal_matmul_tb(void* C, const void* A, const void* B,
    ...
    @autoreleasepool {
        ...
        MPSMatrixMultiplication* mmul = [[MPSMatrixMultiplication alloc] initWithDevice:g_device
            transposeLeft:NO transposeRight:YES resultRows:M resultColumns:N interiorColumns:K alpha:alpha beta:beta];
...
int gollek_metal_matmul_tb_half(void* C, const void* A, const void* B,
    ...
    @autoreleasepool {
        ...
        MPSMatrixDescriptor* descA = [MPSMatrixDescriptor matrixDescriptorWithRows:M columns:K rowBytes:K*sizeof(float) dataType:MPSDataTypeFloat32];
        MPSMatrixDescriptor* descB = [MPSMatrixDescriptor matrixDescriptorWithRows:N columns:K rowBytes:K*b_elem_size dataType:b_mps_type];
        MPSMatrixDescriptor* descC = [MPSMatrixDescriptor matrixDescriptorWithRows:M columns:N rowBytes:N*sizeof(float) dataType:MPSDataTypeFloat32];
    ...
    for (int b = 0; b < B; b++) {
        ...
        for (int blk = 0; blk < num_blocks; blk++) {
            ...
            for (int h = 0; h < H_kv; h++) {
                ...
                float* dst_k = kPtr + ((size_t)blk * block_size * H_kv + h * block_size) * D;
                float* dst_v = vPtr + ((size_t)blk * block_size * H_kv + h * block_size) * D;
        ...
        for (int h = 0; h < H; h++) {
            ...
            for (int t = 0; t < T; t++) {
                ...
                for (int s = 0; s < limit; s++) {
                    int blk = s / block_size;
                    int tok = s % block_size;
                    const float* kh = kPtr + ((size_t)blk * block_size * H_kv + kv_h * block_size + tok) * D;
                    float dot = 0.0f;
                    for (int d = 0; d < D; d++) dot += qh[d] * kh[d];
                ...
                for (int s = 0; s < limit; s++) {
                    int blk = s / block_size;
                    int tok = s % block_size;
                    const float* vh = vPtr + ((size_t)blk * block_size * H_kv + kv_h * block_size + tok) * D;
                    float weight = row[s];
                    for (int d = 0; d < D; d++) oh[d] += weight * vh[d];
                }
            }
